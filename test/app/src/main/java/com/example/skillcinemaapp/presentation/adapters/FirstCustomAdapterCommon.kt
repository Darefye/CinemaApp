...
open class FirstCustomAdapterCommon(
    val onFirstCustomItemClick: (ItemCustom) -> Unit, val onShowAllFirstCustomClick: (View) -> Unit
...
class ShowAllViewHolder(
    val binding: ShowAllItemBinding, val onShowAllItemClick: (View) -> Unit
...
class CustomSelectionViewHolderCommon(
    val binding: ItemFilmBinding, val onItemClick: (ItemCustom) -> Unit
) : RecyclerView.ViewHolder(binding.root) {
    fun bind(item: ItemCustom) {
        with(binding) {
            Glide.with(itemFilmPicture.context).load(item.posterUrlPreview).centerCrop()
                .into(itemFilmPicture)

            itemFilmGenre.text = item.genres.firstOrNull()?.genre

            if (item.watchedStatus != null) {
                itemFilmWatchedStatus.isVisible = item.watchedStatus == true
            } else itemFilmWatchedStatus.isVisible = false

            if (item.nameRu == null && item.nameEn == null && item.nameOriginal == null) {
                itemFilmTitle.isVisible = false
            } else {
                itemFilmTitle.text = when {
